...
public class OrderBookTest {
	...
	public void simplePushTest() {
		...
		orderBook.push(bid2);
		orderBook.push(bid3);
		assert orderBook.print().get(0).equals("price:[999],  orderList:[10, 11, 12]");

		orderBook.push(ask2);
		orderBook.push(ask3);
		assert orderBook.print().get(1).equals("price:[1001],  orderList:[21, 22]");
	}
	...
	public void simpleRemoveTest() {
		...
		assert orderBook.print().get(0).equals("price:[999],  orderList:[11, 12]");
	...
	public void simpleBidMatchTest() {
		orderBook.push(ask1);
		assert orderBook.print().get(0).equals("price:[1000],  orderList:[20]");
		orderBook.push(
				new Order("BTCUSDT", 30, 30, OrderSide.ASK, OrderType.LIMIT, BigDecimal.valueOf(1000),
						  BigDecimal.ONE));
		assert orderBook.print().get(0).equals("price:[1000],  orderList:[20, 30]");
		// match
		List<Trade> trades = orderBook.push(
				new Order("BTCUSDT", 31, 31, OrderSide.BID, OrderType.LIMIT, BigDecimal.valueOf(1000),
						  BigDecimal.ONE));
		assert trades.size() == 1;
		assert orderBook.print().get(0).equals("price:[1000],  orderList:[30]");
	}

	@Test
	public void simpleAskMatchTest() {
		orderBook.push(bid4);
		assert orderBook.print().get(0).equals("price:[1000],  orderList:[13]");
		orderBook.push(
				new Order("BTCUSDT", 30, 30, OrderSide.BID, OrderType.LIMIT, BigDecimal.valueOf(1000),
						  BigDecimal.ONE));
		assert orderBook.print().get(0).equals("price:[1000],  orderList:[13, 30]");
		// match
		List<Trade> trades = orderBook.push(
